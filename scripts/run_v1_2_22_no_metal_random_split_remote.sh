...
OUT_ROOT="${OUT_ROOT:-outputs/experiments/v1_2_22_no_metal_random_split_remote}"
SUMMARY_OUT="${SUMMARY_OUT:-outputs/experiments/v1_2_22_no_metal_random_split_remote_summary}"
LOG_DIR="${LOG_DIR:-outputs/logs}"
RUN_TIMES="${RUN_TIMES:-${LOG_DIR}/run_v1_2_22_no_metal_random_split_times.csv}"
...
RANDOM8_SOIL_SPLIT="${SOIL_SPLIT_PREFIX}B_random_8_2"
RANDOM8_TRANSFER_SPLIT="M_v2_aquatic_to_soil_ptox_no_metal_adapt_B_random_8_2_f100"

ensure_outputs() {
  mkdir -p "$OUT_ROOT" "$SUMMARY_OUT" "$LOG_DIR" "$(dirname "$DB")"
  if [[ ! -s "$RUN_TIMES" ]]; then
...
fold_transfer_split() {
  local fold="$1"
  echo "M_v2_aquatic_to_soil_ptox_no_metal_adapt_E_random_5fold_fold${fold}_f100"
}

fold_soil_split() {
  local fold="$1"
  echo "${SOIL_SPLIT_PREFIX}E_random_5fold_fold${fold}"
}
...
build_soil_splits() {
  echo "[soil-splits] source_table=${SOIL_TABLE} prefix=${SOIL_SPLIT_PREFIX} seed=${SPLIT_SEED}"
  "$PYTHON_BIN" -m qsar_tl.cli generate-experiment-splits \
    --config "$CONFIG" \
    --db "$DB" \
...
run_transfer() {
  ...
  local split="$2"
  local run_seed="$3"
  local out_dir="${OUT_ROOT}/v1.2.22_${run_name}/deep/full/${split}"
...
run_matrix() {
  ...
  for run_seed in $SEEDS; do
    if [[ "$RUN_RANDOM8" == "1" ]]; then
      run_name="${RUN_NAME_PREFIX}no_metal_random8_2_seed${run_seed}_cebin_lw0025_censored_w${cw}"
      run_transfer "$run_name" "$RANDOM8_TRANSFER_SPLIT" "$run_seed" || return $?
    fi
    if [[ "$RUN_5FOLD" == "1" ]]; then
      for fold in $FOLDS; do
        run_name="${RUN_NAME_PREFIX}no_metal_random5fold_fold${fold}_seed${run_seed}_cebin_lw0025_censored_w${cw}"
...
run_smoke() {
  ...
  RUN_TIMES="${LOG_DIR}/run_v1_2_22_no_metal_random_split_smoke_times.csv"
...
echo "[start] $(date -Is) v1.2.22 no-metal random split mode=${MODE} seeds=${SEEDS} folds=${FOLDS}"
case "$MODE" in
  prepare_db)
    prepare_db
    ;;
...
status=$?
echo "[done] $(date -Is) v1.2.22 no-metal random split mode=${MODE} status=${status}"
exit "$status"
