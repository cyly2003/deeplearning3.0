...
OUT_ROOT="${OUT_ROOT:-outputs/experiments/v1_2_26_no_metal_random_split_ablation_remote}"
SUMMARY_OUT="${SUMMARY_OUT:-outputs/experiments/v1_2_26_no_metal_random_split_ablation_remote_summary}"
LOG_DIR="${LOG_DIR:-outputs/logs}"
RUN_TIMES="${RUN_TIMES:-${LOG_DIR}/run_v1_2_26_no_metal_random_split_ablation_times.csv}"

RANDOM8_SPLIT="${RANDOM8_SPLIT:-M_v2_aquatic_to_soil_ptox_no_metal_adapt_B_random_8_2_f100}"
FIVEFOLD_SPLIT_PREFIX="${FIVEFOLD_SPLIT_PREFIX:-M_v2_aquatic_to_soil_ptox_no_metal_adapt_E_random_5fold_fold}"
FIVEFOLD_SPLIT_SUFFIX="${FIVEFOLD_SPLIT_SUFFIX:-_f100}"
RANDOM8_SEEDS="${RANDOM8_SEEDS:-2042}"
FIVEFOLD_SEEDS="${FIVEFOLD_SEEDS:-2042}"
...
run_exists() {
  ...
  local ablation="$2"
  local split="$3"
  local out_dir="${OUT_ROOT}/v1.2.26_${run_name}/deep/${ablation}/${split}"
...
run_module_ablation() {
  ...
  for ablation in $MODULE_ABLATIONS; do
    run_name="${RUN_NAME_PREFIX}no_metal_${split_policy}${fold_label}_ablation_${ablation}_seed${seed}_cebin_lw0025_censored_w${cw}"
...
run_strategy_ablation() {
  ...
  for strategy in $STRATEGY_ABLATIONS; do
    run_name="${RUN_NAME_PREFIX}no_metal_${split_policy}${fold_label}_strategy_${strategy}_seed${seed}"
    case "$strategy" in
      no_source_weighting)
...
run_smoke() {
  ...
  RANDOM8_SEEDS="${SMOKE_RANDOM8_SEEDS:-2042}"
  FIVEFOLD_SEEDS="${SMOKE_FIVEFOLD_SEEDS:-2042}"
  FOLDS="${SMOKE_FOLDS:-1}"
  ...
  RUN_TIMES="${LOG_DIR}/run_v1_2_26_no_metal_random_split_ablation_smoke_times.csv"
  mkdir -p "$OUT_ROOT" "$SUMMARY_OUT" "$LOG_DIR"
  if [[ ! -s "$RUN_TIMES" ]]; then
    echo "kind,split_policy,run_name,ablation,split_name,start_iso,end_iso,duration_seconds,exit_code" > "$RUN_TIMES"
  fi
...
echo "[start] $(date -Is) v1.2.26 no-metal random split ablation mode=${MODE} random8_seeds=${RANDOM8_SEEDS} fivefold_seeds=${FIVEFOLD_SEEDS} folds=${FOLDS}"
case "$MODE" in
  smoke)
    run_smoke
    ;;
...
summarize
echo "[done] $(date -Is) v1.2.26 no-metal random split ablation mode=${MODE}"
