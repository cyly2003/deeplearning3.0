...
FORMAL_ROOT="outputs/experiments/v1_2_46_reference_group_matrix"
OUT_ROOT="${OUT_ROOT:-$FORMAL_ROOT}"
AUDIT_DIR="${AUDIT_DIR:-outputs/audits/v1_2_46_reference_group}"
...
mkdir -p "$AUDIT_DIR" outputs/logs

build_splits() {
  "$PYTHON" scripts/build_v1_2_46_reference_group_splits.py \
    --db "$DB" \
...
exec 8>outputs/logs/v1_2_46_reference_group_matrix.lock
if ! flock -n 8; then
  echo "[blocked] v1.2.46 reference-group matrix is already queued or running" >&2
  exit 75
fi
# Serialize behind the retained v1.2.40/v1.2.44 source controllers.
exec 9>outputs/logs/v1_2_44_second_layer_matrix.lock
flock 9
exec 4>outputs/logs/v1_2_40_paired_mass_molar_matrix.lock
flock 4
...
cell_contract() {
  ...
  case "$cell" in
    M00)
      ...
      RUN_NAME="M00_reference_group从头训练_种子${seed}"
    ...
    M10)
      ...
      RUN_NAME="M10_reference_group仅水相预训练_种子${seed}"
    ...
    M11U)
      ...
      RUN_NAME="M11U_reference_group完整三阶段全参数微调_种子${seed}"
      CELL_STAGE1_EPOCHS="$STAGE1_EPOCHS"
      CELL_STAGE2_EPOCHS="$STAGE2_EPOCHS"
      SOURCE_WEIGHTING="tanimoto_to_finetune"
      ;;
    *) echo "Unknown v1.2.46 cell: $cell" >&2; return 2 ;;
  esac
  RUN_DIR="$RUN_ROOT/v1.2.46_${RUN_NAME}/deep/full/$CELL_SPLIT"
}
...
validate_cell() (
  ...
  "$PYTHON" scripts/validate_v1_2_46_reference_group_run.py \
    --run-dir "$RUN_DIR" \
    --cell "$cell" \
    --seed "$seed" \
...
run_cell() {
  ...
  cell_contract "$cell" "$seed"
  log="outputs/logs/v1_2_46_${MODE}_${cell}_seed${seed}.log"
...
"$PYTHON" scripts/summarize_v1_2_46_reference_group_matrix.py \
  --root "$RUN_ROOT" \
