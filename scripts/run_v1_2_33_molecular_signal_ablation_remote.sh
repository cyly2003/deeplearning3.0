...
OUT_ROOT="${OUT_ROOT:-outputs/experiments/v1_2_33_molecular_signal_ablation_remote}"
SUMMARY_OUT="${SUMMARY_OUT:-outputs/experiments/v1_2_33_molecular_signal_ablation_remote_summary}"
LOG_DIR="${LOG_DIR:-outputs/logs}"
RUN_TIMES="${RUN_TIMES:-${LOG_DIR}/run_v1_2_33_molecular_signal_ablation_times.csv}"
...
ABLATIONS="${ABLATIONS:-full no_descriptors no_fingerprint descriptors_only}"
...
run_exists() {
  ...
  local ablation="$2"
  local split="$3"
  local out_dir="${OUT_ROOT}/v1.2.33_${run_name}/deep/${ablation}/${split}"
...
run_split() {
  ...
  for seed in $SEEDS; do
    for ablation in $ABLATIONS; do
      run_name="${RUN_NAME_PREFIX}${split_policy}${fold_label}_${ablation}_seed${seed}_molecular_signal_ablation"
...
run_smoke() {
  ...
  FOLDS="${SMOKE_FOLDS:-1}"
  ABLATIONS="${SMOKE_ABLATIONS:-no_descriptors}"
  ...
  RUN_TIMES="${LOG_DIR}/run_v1_2_33_molecular_signal_ablation_smoke_times.csv"
  mkdir -p "$OUT_ROOT" "$SUMMARY_OUT" "$LOG_DIR"
  if [[ ! -s "$RUN_TIMES" ]]; then
    echo "kind,split_policy,run_name,ablation,split_name,start_iso,end_iso,duration_seconds,exit_code" > "$RUN_TIMES"
  fi
...
echo "[start] $(date -Is) v1.2.33 molecular-signal ablation mode=${MODE} seeds=${SEEDS} folds=${FOLDS} ablations=${ABLATIONS}"
case "$MODE" in
  smoke)
    run_smoke
    ;;
...
summarize
echo "[done] $(date -Is) v1.2.33 molecular-signal ablation mode=${MODE}"
