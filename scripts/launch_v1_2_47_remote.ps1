...
$allowlist = @(
    "scripts/build_v1_2_47_scaffold_family_splits.py",
    "scripts/validate_v1_2_47_scaffold_family_run.py",
    "scripts/summarize_v1_2_47_scaffold_family_matrix.py",
    "scripts/run_v1_2_47_scaffold_family_matrix_remote.sh",
    "scripts/launch_v1_2_47_remote.ps1",
    "tests/test_v1_2_47_scaffold_family_matrix.py",
    "tests/test_v1_2_47_scaffold_runner.py"
)
...
if ($Sync) {
    ...
    Write-Host "Synced v1.2.47 allowlist only."
}

if ($Preflight) {
    $pythonFiles = ($allowlist | Where-Object { $_ -like "*.py" }) -join " "
    $body = @(
        "set -euo pipefail",
        "cd $remoteProjectQuoted",
        "$pythonQuoted -m py_compile $pythonFiles",
        "bash -n scripts/run_v1_2_47_scaffold_family_matrix_remote.sh",
        "$pythonQuoted -m pytest tests/test_v1_2_47_scaffold_family_matrix.py tests/test_v1_2_47_scaffold_runner.py tests/test_v1_2_46_reference_group_matrix.py -q",
        "REBUILD_SPLITS=1 PARALLEL_JOBS=2 bash scripts/run_v1_2_47_scaffold_family_matrix_remote.sh build"
    ) -join "; "
    Invoke-RemoteBash -Body $body
    Write-Host "Remote v1.2.47 tests and locked split preflight completed."
}

if ($Launch) {
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $controllerLog = "outputs/logs/v1_2_47_scaffold_family_gate_$stamp.log"
    $pidFile = "outputs/logs/v1_2_47_scaffold_family_gate_$stamp.pid"
    $gate = (
        "set -euo pipefail; " +
        "REBUILD_SPLITS=0 PARALLEL_JOBS=2 bash scripts/run_v1_2_47_scaffold_family_matrix_remote.sh smoke; " +
        "REBUILD_SPLITS=0 PARALLEL_JOBS=2 bash scripts/run_v1_2_47_scaffold_family_matrix_remote.sh formal"
    )
    $body = (
