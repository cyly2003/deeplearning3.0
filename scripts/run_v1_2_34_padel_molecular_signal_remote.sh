...
CONFIG="${CONFIG:-configs/experiment.remote.easyai.padel.yaml}"
...
RUN_VERSION="${RUN_VERSION:-v1.2.34}"
EXPERIMENT_LABEL="${EXPERIMENT_LABEL:-padel_descriptor}"
OUT_ROOT="${OUT_ROOT:-outputs/experiments/v1_2_34_padel_descriptor_remote}"
SUMMARY_OUT="${SUMMARY_OUT:-outputs/experiments/v1_2_34_padel_descriptor_remote_summary}"
LOG_DIR="${LOG_DIR:-outputs/logs}"
RUN_TIMES="${RUN_TIMES:-${LOG_DIR}/run_v1_2_34_padel_descriptor_times.csv}"
...
run_smoke() {
  ...
  RUN_TIMES="${LOG_DIR}/run_v1_2_34_padel_descriptor_smoke_times.csv"
  mkdir -p "$OUT_ROOT" "$SUMMARY_OUT" "$LOG_DIR"
  if [[ ! -s "$RUN_TIMES" ]]; then
    echo "kind,split_policy,run_name,ablation,split_name,start_iso,end_iso,duration_seconds,exit_code" > "$RUN_TIMES"
  fi
